...
class MyApp(BaseRenderer, ShowBase):
    ...
    def setupScene(self, client):
        """Init pybullet scene"""
        client.setAdditionalSearchPath('../models')
        # table = client.loadURDF("table/table.urdf")
        # client.resetBasePositionAndOrientation(
        #     table, [0.4, 0.04, -0.7], [0, 0, 0, 1])

        kuka = client.loadURDF('buff_digit/prost_digit_hover.urdf', [0,0,0], useFixedBase=True)
        # loadSDF("kuka_iiwa/kuka_with_gripper2.sdf")[0]
        client.resetBasePositionAndOrientation(
